...
//	}
//
//	int required_new_node_count = ( (seq + length - window->lastSeq) / ALTUS_WQ_NODE_BYTE_COUNT ) + 1;//this parameter not reliable.
...
//
//		while (leftLen != 0) {
//			unsigned int writelen = currentNodeSeq + ALTUS_WQ_NODE_BYTE_COUNT - leftSeq;
...
//				currentNodeSeq += ALTUS_WQ_NODE_BYTE_COUNT;
//				continue;
//			}
//			if (leftLen < writelen) {
//				writelen = leftLen;
...
//			leftLen -= writelen;
//
//			if (leftLen != 0 || leftSeq == currentNodeSeq + ALTUS_WQ_NODE_BYTE_COUNT) {
//				altusWQNewNode(window, pool);
//				currentNode = currentNode->to_tail;
//				currentNodeSeq += ALTUS_WQ_NODE_BYTE_COUNT;
//			}
...
//			if (seqDiff > ALTUS_WQ_NODE_BYTE_COUNT) {
//				window->safe_tail->validBlockCount = ALTUS_WQ_NODE_BYTE_COUNT;
//				window->lastSeq = SafeNodeSeq + ALTUS_WQ_NODE_BYTE_COUNT;
//				window->safe_len++;
//				window->unsafe_len--;
...
//				continue;
//			}
//			else if (seqDiff == ALTUS_WQ_NODE_BYTE_COUNT) {
...
//
//		if (window->headDataOffset == ALTUS_WQ_NODE_BYTE_COUNT) {
//			currentNode = currentNode->to_tail;
...
//	for (; deltaSeq >= ALTUS_WQ_NODE_BYTE_COUNT; deltaSeq -= ALTUS_WQ_NODE_BYTE_COUNT) {
...
//	for (int i = 0; i < ALTUS_NODE_SIZE; i++) {
